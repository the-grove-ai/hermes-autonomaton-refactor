"""Grove Kaizen — Curator (skill maintenance orchestrator).

TPS framing (Grove Autonomaton Pattern Draft 1.4, Commitment 5.3):
Kaizen is the Recommender — the third element of the Jidoka / Andon /
Kaizen triplet. Jidoka detects deviation; Andon halts execution at
sovereignty boundaries; Kaizen recommends improvements. The Curator is
Kaizen's skill-maintenance arm: it reviews agent-created skills and
proposes lifecycle transitions.

This module is a verbatim copy of ``agent/curator.py``, brought into the
``grove.kaizen`` namespace by Sprint 06b (kaizen-foundation-v1). The
original ``agent.curator`` remains the canonical implementation for
existing consumers (``cli.py``, ``gateway/run.py``) until a later sprint
renames those imports. Functionality here is byte-identical to the
original — only this docstring differs.
