...
def _local_config_path() -> Path:
    ...
    Always returns $GROVE_HOME/honcho.json so each profile/instance gets
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Autonomaton persistent cross-session memory.")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Autonomaton' identity in Honcho. Seed with 'hermes honcho identity <file>'.")
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. Autonomaton asks Honcho's AI peer a question")
...
def cmd_migrate(args) -> None:
    ...
    print("\nHoncho migration: OpenClaw native memory → Autonomaton\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Autonomaton persistent memory")
    ...
    if user_files:
        ...
        print("  (Autonomaton calls migrate_memory_files() on first session init.)")
        print()
    ...
    print()
    print("  In Autonomaton, they are seeded once into Honcho's AI peer through the")
    ...
    print("    Autonomaton:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Autonomaton:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Autonomaton:   Honcho observes every message and updates representations")
    ...
    print("    Autonomaton:   per-session by default — each run gets its own session")
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Autonomaton Honcho",
    )
