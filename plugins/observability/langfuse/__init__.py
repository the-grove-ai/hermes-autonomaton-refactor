...
Required env vars (set via ``hermes tools`` or ~/.grove/.env):
  GROVE_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  GROVE_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  GROVE_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  GROVE_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  GROVE_LANGFUSE_RELEASE     - release/version tag
  GROVE_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  GROVE_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  GROVE_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
_LANGFUSE_KEY_PREFIXES: Dict[str, str] = {
    "GROVE_LANGFUSE_PUBLIC_KEY": "pk-lf-",
    "GROVE_LANGFUSE_SECRET_KEY": "sk-lf-",
}
...
def _debug_enabled() -> bool:
    return _env_bool("GROVE_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    public_key = _env("GROVE_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("GROVE_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    placeholder_issues = [
        msg
        for msg in (
            _validate_langfuse_key("GROVE_LANGFUSE_PUBLIC_KEY", public_key),
            _validate_langfuse_key("GROVE_LANGFUSE_SECRET_KEY", secret_key),
        )
        if msg
    ]
    if placeholder_issues:
        logger.warning(
            ...
            "or unset GROVE_LANGFUSE_PUBLIC_KEY / GROVE_LANGFUSE_SECRET_KEY to "
    ...
    base_url = _env("GROVE_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("GROVE_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("GROVE_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("GROVE_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid GROVE_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("GROVE_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
...
def _cost_details_from_declared(
    input_tokens: int,
    output_tokens: int,
    ...
    *,
    ...
    """Per-type cost_details from the DECLARED per-mtok rates handed to us.

    binding-opacity-v1: this plugin is a pure CONSUMER of the cost facts
    threaded through the hook payload — it imports no router, parses no slug,
    and looks up no name-keyed pricing table. Cache tokens are billed at the
    declared input rate (the binding declares no separate cache tier), matching
    ...
    if input_tokens:
        cost_details["input"] = float(Decimal(input_tokens) * input_rate / _ONE_M)
    if output_tokens:
        cost_details["output"] = float(Decimal(output_tokens) * output_rate / _ONE_M)
    if cache_read_tokens:
        cost_details["cache_read_input_tokens"] = float(Decimal(cache_read_tokens) * input_rate / _ONE_M)
    if cache_write_tokens:
        cost_details["cache_creation_input_tokens"] = float(Decimal(cache_write_tokens) * input_rate / _ONE_M)
    return cost_details
...
def _usage_and_cost(
    ...
    *,
    ...
    try:
        ...
        # Cost is DECLARED, threaded from run_agent's model_facts through the
        # hook payload. Absent declared rates -> omit cost_details entirely
        # (unknown-and-loud), never guess from the slug.
        cost_details = _cost_details_from_declared(
...
def on_post_llm_call(*, task_id: str = "", session_id: str = "", provider: str = "", base_url: str = "",
    ...
    if response is not None:
        ...
        # Per-type cost from the DECLARED rates threaded through the hook
        # payload — this plugin never parses the slug or reads a pricing table.
        # Absent declared rates -> no cost_details (unknown-and-loud).
        cost_details = _cost_details_from_declared(
            _input,
            _output,
            _cache_read,
