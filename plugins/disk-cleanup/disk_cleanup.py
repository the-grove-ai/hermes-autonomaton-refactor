...
Rules:
  ...
  - empty dirs    → always delete (under GROVE_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly GROVE_HOME and /tmp/hermes-*
Never touches: ~/.grove/logs/ or any system directory.
"""
...
try:
    ...
    def get_hermes_home() -> Path:  # type: ignore[no-redef]
        ...
        return Path(val).resolve() if val else (Path.home() / ".grove").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$GROVE_HOME/logs/``."""
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$GROVE_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under GROVE_HOME or ``/tmp/hermes-*``.
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside GROVE_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under GROVE_HOME (but leave GROVE_HOME itself and
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under GROVE_HOME (e.g. /tmp/hermes-*) — fall through.
        pass
