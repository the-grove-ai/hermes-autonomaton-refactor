...
Uses a file-based lock (~/.grove/cron/.tick.lock) so only one tick
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("GROVE_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid GROVE_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within GROVE_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against GROVE_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    # escape — scripts MUST reside within GROVE_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    os.environ["GROVE_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed GROVE_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads GROVE_SESSION_PLATFORM
    #     and GROVE_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via GROVE_CRON_AUTO_DELIVER_* below.
    ...
    #     send_message gate read GROVE_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the GROVE_CRON_AUTO_DELIVER_* vars set
    # below, so clearing GROVE_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "GROVE_CRON_AUTO_DELIVER_PLATFORM",
        "GROVE_CRON_AUTO_DELIVER_CHAT_ID",
        "GROVE_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        if delivery_target:
            _VAR_MAP["GROVE_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["GROVE_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["GROVE_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        model = job.get("model") or os.getenv("GROVE_MODEL") or ""
        ...
        prefill_file = os.getenv("GROVE_PREFILL_MESSAGES_FILE", "") or _cfg.get("prefill_messages_file", "")
        if prefill_file:
        ...
        try:
            # Do not inject GROVE_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = Dispatcher(session_db=_session_db, agent_kwargs=dict(
            ...
            # GROVE_HOME. When a workdir is configured, also inject project
        ...
        # override via GROVE_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("GROVE_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid GROVE_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        # Set GROVE_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("GROVE_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid GROVE_CRON_MAX_PARALLEL value; defaulting to unbounded")
