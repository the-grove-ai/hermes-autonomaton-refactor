...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Hermes also honours GROVE_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("GROVE_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
# binding-opacity-v1 P4b 1c — grok_supports_reasoning_effort() and its
# _GROK_EFFORT_CAPABLE_PREFIXES allowlist DELETED. Whether an xAI model accepts
# the reasoning.effort dial is now a declared physics fact
# (model_facts.reasoning_support), resolved on the routing binding and read by
# the codex transport off the params carrier — never inferred from a "grok-*"
# name allowlist. This was the last dispatch-path importer of the symbol; the
# opacity guard pins that no module re-adds the import (a substring allowlist on
# the slug is the exact anti-pattern the arc removes).
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "github-copilot"}:
        try:
