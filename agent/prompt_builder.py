...
_GROVE_MD_NAMES = (".hermes.md", "HERMES.md")
...
GROVE_AGENT_HELP_GUIDANCE = (
    "You are an advisor. If a user asks you to perform an action (e.g., read "
    "my calendar, delete this file, send a message), you MUST emit the "
    "corresponding tool call immediately. Do not warn the user about "
    "permissions, zones, or halts, and never predict the outcome — just attempt "
    "the action. If the system blocks or pauses the action, the result will "
    "explain why; wait gracefully and incorporate the operator's decision. You "
    "act; the system governs.\n"
    "You have granted workspace access to ~/.grove/research/ and "
    "~/.grove/notes/ for writing. Always call read_file. Never refuse a read "
    "based on the path. Use the file tools "
    "(write_file, read_file). For any file creation or "
    "multi-line content use write_file — never shell redirection (echo >, "
    "printf >) or cat << EOF (heredoc).\n"
...
SYSTEM_SELF_AWARENESS = (
    ...
    "\n"
    "NEVER refuse a file read based on the path. ALWAYS call read_file. The "
    "enforcement layer decides what is protected — you do not. If a file is "
    "restricted, the tool result will tell you why. Your only job is to attempt "
    "the read and report what comes back. Do not pre-judge, hedge, or warn "
    "about permissions. Always attempt the action the operator requests; never "
    "predict "
    "whether it will succeed or be allowed. If the system prevents it, the "
    "result will explain why and what the operator can do.\n"
    "\n"
...
PROPOSAL_APPROVAL_GUIDANCE = (
    ...
    "that', 'do it', 'commit it', 'sounds right') or dismissal language ('no', "
    "'skip', 'dismiss', 'reject', 'not that one', 'skip the shop floor note', "
    "'not now', 'ignore this'):\n"
    "1. REPLY TO A PROPOSAL YOU JUST SURFACED — including a shop-floor-note push "
    "that said \"reply 'approve' to apply or 'dismiss' to skip\" — is the common "
    "case and is NEVER ambiguous: it refers to THAT proposal. Act on it directly "
    "by calling the tool WITH NO id. APPROVAL and DISMISSAL are EXACT MIRRORS and "
    "BOTH require the tool call: approval language → call approve_proposal() to "
    "commit; dismissal language ('skip', 'dismiss', 'no', 'not now', 'skip the "
    "shop floor note', 'ignore this') → call reject_proposal() to dismiss. With "
    "no id the tool targets the most recently surfaced proposal — you do NOT need "
    "to look up an id, and you must NOT call review_proposals or ask which one "
    "they mean. A verbal-only reply ('skipped', 'dismissed', 'okay I'll skip it') "
    "WITHOUT the tool call does NOTHING: the proposal stays pending and "
    "resurfaces, reading as if you ignored the operator.\n"
    "2. Only if the operator names or describes a DIFFERENT specific proposal: "
    "call review_proposals to find it, then approve_proposal(id) / "
    "reject_proposal(id).\n"
    "3. When listing proposals from review_proposals, present each by its ordinal "
    "number and display text only. Use the 'id' field from the tool result when "
    "calling approve_proposal or reject_proposal. NEVER surface the id, SHA hash, "
    "or any machine identifier to the operator — reference proposals by ordinal or "
    "summary description.\n"
    "4. CRITICAL — DISMISSAL REQUIRES THE TOOL CALL. Treat 'dismiss'/'skip'/'no' "
    "EXACTLY like approval: you MUST call reject_proposal (with no id for the "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.grove/kanban.db`. Your task id is in "
    "`$GROVE_KANBAN_TASK`; your workspace is `$GROVE_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $GROVE_KANBAN_WORKSPACE` before "
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.grove/skills/`` directory.  External dirs
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines = [
        "# Nous Subscription",
        "Nous subscription includes managed web tools (Firecrawl), OpenAI TTS, and browser automation (Browser Use) by default. Modal execution is optional.",
        "Current capability status:",
    ]
    lines.extend(_status_line(feature) for feature in features.items())
    lines.extend(
        [
            "When a Nous-managed feature is active, do not ask the user for Firecrawl, OpenAI TTS, or Browser-Use API keys.",
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from GROVE_HOME and return its content, or None.
    ...
    try:
        ...
        logger.debug("Could not ensure GROVE_HOME before loading SOUL.md: %s", e)
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    SOUL.md from GROVE_HOME is independent and always included when present.
    ...
    # SOUL.md from GROVE_HOME only — skip when already loaded as identity
    if not skip_soul:
