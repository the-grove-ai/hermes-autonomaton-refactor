...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("GROVE_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("GROVE_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set GROVE_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind GROVE_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds GROVE_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("GROVE_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $GROVE_HOME.
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("GROVE_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = env_var_enabled("GROVE_INTERACTIVE")
    ...
    if is_gateway or env_var_enabled("GROVE_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("GROVE_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}
