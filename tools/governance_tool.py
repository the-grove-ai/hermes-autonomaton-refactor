...
GOVERNANCE_CHANGE_SCHEMA = {
    ...
    "description": (
        ...
        "is sealed against the governed-writer registry at Stage 04 — the "
        "registry resolves the target AT SEAL TIME, per surface; there is no "
        "hardcoded target name set. Registered surfaces include capability "
        "ADMISSION state: target ~/.grove/capabilities/state/<record_id>.yaml "
        "with a canonical admission doc {id, intents, tiers} to propose an "
        "intents/tiers change (write_admission_state executes it on operator "
        "approval — no deploy needed; definitions under config/ stay "
        "git+deploy). The sealed claim is queued for operator approval and "
        "executed by the approval executor — never written directly. A "
        "surface with no registered writer, or a repo definition file, is "
        "refused loudly. Provide the FULL new file content and a rationale; "
        "the seal and the write are both logged. For fleet output directories "
