...
try:
    from hermes_cli import __version__ as GROVE_VERSION
except Exception:
    GROVE_VERSION = "0.0.0"
...
class HermesACPAgent(acp.Agent):
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="hermes-agent", version=GROVE_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        # not the executor's. Also set GROVE_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue GROVE_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind GROVE_SESSION_KEY for this session so per-session caches
            ...
            try:
                ...
                # Restore GROVE_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("GROVE_INTERACTIVE", None)
                else:
                    os.environ["GROVE_INTERACTIVE"] = previous_interactive
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (GROVE_SESSION_KEY in
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Hermes Agent v{GROVE_VERSION}"
