...
**Test surface:** the Grove governance suite runs deterministically — reproduce with `scripts/run_tests.sh tests/grove/` rather than trust a frozen count here. A small set of tests are `xfail`-quarantined with tracked, greppable reasons (declared, never hidden — the disposition rides a named follow-up). The inherited upstream suite (~23.9k tests) runs alongside it. A dedicated CI job — `cold-start (mocked provider)` — proves a cold instance materializes to a contract-valid state and boots + routes one turn against a **mocked** provider, with the provider keys blanked (no network); it does **not** prove install-and-go. That live claim is carried separately by the `GROVE_LIVE_TESTS` cold-start smoke (`tests/grove/test_cold_start_live_smoke.py`), which runs where real credentials exist. Please report anything you find. Contributions welcome.
...
- **Surface scope.** The gateway ships two adapters: Telegram and the OpenAI-compatible API server. The upstream Slack, Discord, WhatsApp, and Matrix adapters were severed from this fork (they were never tested against the Grove governance layer). Most testing has been done through the CLI, Telegram, and the API-server surfaces.
