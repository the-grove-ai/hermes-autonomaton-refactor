...
**Test surface (v0.2.0):** Grove governance suite (`tests/grove/`): **2,587 / 2,608 passing** — reproduce with `scripts/run_tests.sh tests/grove/`. The 21 known failures are pre-existing and under cleanup (verified to fail identically before this release's work): a shared test-fixture bug (`SimpleNamespace` stubs missing a cooldown attribute), one `review_proposals` output-schema mismatch, a handful of golden-snapshot / offer-parity tests, and bundled-skill frontmatter format checks — none are governance-behavior regressions. The full inherited upstream suite (~24.7k tests) runs via `scripts/run_tests.sh`; its residual is documented environment-gated skips (discord.py 2.x mocks, macOS Keychain, Linux systemd, PTY) and upstream-divergence skips. Live CLI integration: T1–T31. Telegram gateway: 510 tests.
...
**Recent hardening (v0.2.0).** Three governance defects surfaced by live operation on the gateway were closed and confirmed end-to-end by operator smoke tests on the production surface: (1) the generic file tools now report a governed-path block honestly instead of the model narrating a write that never happened — surfaced even when the turn exits through a sovereignty halt; (2) Green-zone read tools (file search, Workspace and Notion reads) are admitted on every turn rather than gated behind narrow intent matches; (3) the proactive "crystallized insight" surface is now relevance-gated to the turn, deduplicated across sessions, and carries a distinct soft-dismiss that does not poison the extractor's rejection memory. Each shipped as a self-contained sprint — discover, approval-gate, implement, verify — the same loop the architecture runs on everything else.
