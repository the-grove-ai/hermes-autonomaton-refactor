...
def _translate_one_server(
    ...
    for key in hermes_cfg:
        ...
        elif key not in _KNOWN_GROVE_KEYS:
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (GROVE_HOME, PYTHONPATH) could
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a GROVE_HOME pointing
    ...
    ``~/.grove`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    GROVE_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # GROVE_HOME passes through IF SET so the MCP subprocess sees the same
    ...
    # codex's subprocess to inherit whatever GROVE_HOME its launcher sets
    ...
    # — that would override the launcher's GROVE_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("GROVE_HOME", tmp_path) would
    ...
    hermes_home = os.environ.get("GROVE_HOME") or ""
    if hermes_home and _looks_like_test_tempdir(hermes_home):
        hermes_home = ""
    if hermes_home:
        env["GROVE_HOME"] = hermes_home
    ...
    env["GROVE_QUIET"] = "1"
    env["GROVE_REDACT_SECRETS"] = env.get("GROVE_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    Args:
        hermes_config: full ~/.grove/config.yaml dict
