...
- ~/.grove/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.grove/.env         - API keys and secrets
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "GROVE_LANGFUSE_ENV",
    "GROVE_LANGFUSE_RELEASE",
    "GROVE_LANGFUSE_SAMPLE_RATE",
    "GROVE_LANGFUSE_MAX_CHARS",
    "GROVE_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("GROVE_MANAGED", "").strip()
    if raw:
...
def is_managed() -> bool:
    ...
    Two signals: the GROVE_MANAGED env var (set by the systemd service),
    or a .managed marker file in GROVE_HOME (set by the NixOS activation
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from GROVE_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, hermes_bin
    or None if container mode is not active, we're already inside the
    container, or GROVE_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("GROVE_DEV") == "1":
        return None
...
def _secure_dir(path):
    ...
    The mode can be overridden via the GROVE_HOME_MODE environment variable
    (e.g. GROVE_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse GROVE_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("GROVE_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    """
    # Explicit opt-out
    if os.environ.get("GROVE_CONTAINER") or os.environ.get("GROVE_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set GROVE_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        #  "/home/user/.grove/cache/documents:/output"]
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.grove/checkpoints/`` size (MB).  When
    ...
    "display": {
        ...
        # users aren't surprised.  GROVE_TUI_RESUME=<id> always wins.
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.grove/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "memory": {
        ...
        # Set to the name of an installed memory-provider plugin to activate.
    ...
    # always goes to ~/.grove/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${GROVE_SKILL_DIR} and ${GROVE_SESSION_ID} in SKILL.md
    ...
    "curator": {
        ...
        # skipped), snapshot ~/.grove/skills/ into
        # ~/.grove/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *grove-autonomaton*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (GROVE_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.grove/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or GROVE_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via GROVE_CRON_MAX_PARALLEL env var.
    ...
    # Logging — controls file logging to ~/.grove/logs/.
    ...
    # Session storage — controls automatic cleanup of ~/.grove/telemetry.db.
    # telemetry.db accumulates every session, message, tool call, and FTS5 index
    ...
    "sessions": {
        ...
        # telemetry.db itself, so it's shared across all processes.
        "min_interval_hours": 24,
    },
    ...
    "updates": {
        # Run a full ``hermes backup``-style zip of GROVE_HOME before every
        # ``hermes update``.  Backups land in ``<GROVE_HOME>/backups/`` and
        # can be restored with ``hermes import <path>``.  Off by default —
        # on large GROVE_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<GROVE_HOME>/lsp/bin/`` on first use.
...
OPTIONAL_ENV_VARS = {
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. GROVE_BOT)",
    ...
    # GROVE_TOOL_PROGRESS and GROVE_TOOL_PROGRESS_MODE are deprecated —
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("GROVE_HOME", "~/.grove")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("GROVE_TOOL_PROGRESS")
            old_mode = get_env_value("GROVE_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from GROVE_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from GROVE_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("GROVE_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from GROVE_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$GROVE_HOME/plugins/`` for currently installed user plugins.
    ...
    #   3. Creates `~/.grove/logs/curator/` if missing (belt-and-suspenders
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.grove/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.grove/config.yaml.
    ...
    (which change ``GROVE_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
        ensure_hermes_home()
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.grove/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.grove/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.grove/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.grove/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.grove/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.grove/.env into os.environ. Returns count of vars updated.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.grove/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Autonomaton Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
