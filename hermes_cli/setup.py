...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Autonomaton Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    # TTS — show configured provider. After the voice-backend severance the
    # built-in file-TTS backend is Edge; the managed Nous subscription path
    # (OpenAI TTS billed server-side) is the only other built-in offer.
...
def _setup_tts_provider(config: dict):
    ...
    After the voice-backend severance, ``edge`` is the only built-in
    file-TTS backend. Users who want another engine declare a command-type
    provider under ``tts.providers.<name>`` in config.yaml. The managed Nous
    subscription OpenAI TTS path (billed server-side via the gateway) is
    offered when the subscription is active.
    """
    ...
    provider_labels = {
        "edge": "Edge TTS",
        "openai": "OpenAI TTS (managed Nous subscription)",
    }
    ...
    print_info(
        "Custom engines: declare a command-type provider under "
        "tts.providers.<name> in config.yaml."
    )
    print()
    ...
    choices.append("Edge TTS (free, cloud-based, no setup needed)")
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take "
                "precedence until removed from ~/.grove/.env."
            )
...
def _write_slack_manifest_and_instruct():
    ...
    try:
        ...
        manifest = _build_full_manifest(
            bot_name="Hermes",
            bot_description="Your Autonomaton agent on Slack",
        )
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Autonomaton config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Autonomaton.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Autonomaton's yolo mode.", Colors.YELLOW))
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Autonomaton Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
