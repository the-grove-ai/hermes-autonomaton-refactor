...
TIPS = [
    ...
    "GROVE_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    "The plan skill saves markdown plans under .grove/plans/ in the active workspace.",
    ...
    "SOUL.md at ~/.grove/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    ...
    "GROVE_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    ...
    "Cron scripts live in ~/.grove/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.grove/SOUL.md to customize.",
    ...
    "The skills audit log at ~/.grove/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at GROVE_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "GROVE_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in GROVE_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.grove/interrupt_debug.log with timestamps.",
    ...
    "GROVE_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via hermes tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.grove/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.grove/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    # --- TUI & Dashboard ---
    'GROVE_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "GROVE_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'Drop a YAML in ~/.grove/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.grove/dashboard-plugins/ — no npm build required.',
    ...
    '/reload-skills re-scans ~/.grove/skills/ so drop-in skills appear without restarting the session.',
    ...
    'hermes chat --ignore-user-config skips ~/.grove/config.yaml — reproducible bug reports and CI runs.',
    ...
    'GROVE_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'GROVE_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.grove/plugins/ — trust-gated by design.',
    "GROVE_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'GROVE_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'GROVE_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'GROVE_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'GROVE_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'GROVE_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'GROVE_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'GROVE_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'GROVE_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.grove/dashboard-plugins/.',
]
