...
#   2. ~/.grove/node/ from a prior Hermes-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.grove/node/ (always works, zero shell rc edits)
#
# Usage:
#   source scripts/lib/node-bootstrap.sh
#   ensure_node   # returns 0 on success, non-zero on failure
#   if [ "$GROVE_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   GROVE_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   GROVE_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   GROVE_HOME               (default: $HOME/.hermes)
# ============================================================================

GROVE_NODE_MIN_VERSION="${GROVE_NODE_MIN_VERSION:-20}"
GROVE_NODE_TARGET_MAJOR="${GROVE_NODE_TARGET_MAJOR:-22}"
GROVE_HOME="${GROVE_HOME:-$HOME/.hermes}"
GROVE_NODE_AVAILABLE=false
...
_nb_have_modern_node() {
    ...
    [ "$(_nb_node_major)" -ge "$GROVE_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $GROVE_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$GROVE_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$GROVE_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $GROVE_NODE_TARGET_MAJOR..."
    proto install node "$GROVE_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $GROVE_NODE_TARGET_MAJOR..."
    nvm install "$GROVE_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$GROVE_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${GROVE_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${GROVE_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${GROVE_NODE_TARGET_MAJOR}.x/"
    local tarball
    tarball=$(curl -fsSL "$index_url" \
        | grep -oE "node-v${GROVE_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
        | head -1)
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${GROVE_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $GROVE_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
    ...
    _nb_log "Extracting to $GROVE_HOME/node/..."
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $GROVE_HOME/node/"
    return 0
}
...
ensure_node() {
    ...
    _nb_try_fnm   && { GROVE_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { GROVE_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { GROVE_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { GROVE_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { GROVE_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { GROVE_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $GROVE_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
