...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("GROVE_HOME", os.path.expanduser("~/.grove")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("GROVE_HOME", Path.home() / ".grove")) / "skills" / "red-teaming" / "godmode"
...
GROVE_HOME = Path(os.getenv("GROVE_HOME", Path.home() / ".grove"))
CONFIG_PATH = GROVE_HOME / "config.yaml"
PREFILL_PATH = GROVE_HOME / "prefill.json"
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.grove/prefill.json."""
