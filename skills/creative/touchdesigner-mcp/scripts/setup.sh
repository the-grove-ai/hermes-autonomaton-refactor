...
GROVE_HOME_DIR="${GROVE_HOME:-$HOME/.hermes}"
GROVE_CFG="${GROVE_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
if [[ ! -f "$GROVE_CFG" ]]; then
    echo -e " ${FAIL} Hermes config not found at ${GROVE_CFG}"
    manual_steps+=("Create ${GROVE_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$GROVE_CFG" 2>/dev/null; then
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${GROVE_CFG} manually"); }
