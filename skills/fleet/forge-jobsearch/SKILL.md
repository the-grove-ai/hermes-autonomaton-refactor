...
The slug is `YYMMDD-company-role` (e.g., 260702-lilly-ai-orch-pm), lowercase, hyphenated. The three assets are the same in both modes: `resume.md` and `cover-letter.md` (markdown — the publish step converts them to Google Docs), and `meta.json` = the routing metadata the Publish step needs: `{"company": "<Company>", "role": "<Role>", "slug": "<slug>"}`. The Publish handler also reads `row_id` (the Notion page id of the dispatched row) from meta.json to update the Notion row — in headless fleet runs the RUNTIME writes row_id at emit; never author it. (INTERACTIVE mode only: include `row_id` yourself — no runtime bind exists there.) meta.json is not a draft asset — only resume.md and cover-letter.md are published as Google Docs.
...
2. The portal link: http://100.102.6.70:8642/portal#fragments/fleet/forge-jobsearch/<slug>/
