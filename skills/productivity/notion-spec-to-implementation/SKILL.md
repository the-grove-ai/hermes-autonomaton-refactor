...
1. **Find spec**: Use `mcp_notion_notion_search` to locate specification page
2. **Fetch spec**: Use `mcp_notion_notion_fetch` to read specification content
3. **Extract requirements**: Parse and structure requirements from spec
4. **Create plan**: Use `mcp_notion_notion_create_pages` for implementation plan
5. **Find task database**: Use `mcp_notion_notion_search` to locate tasks database
6. **Create tasks**: Use `mcp_notion_notion_create_pages` for individual tasks in task database
7. **Track progress**: Use `mcp_notion_notion_update_page` to log progress and update status
...
```
1. Search for spec:
   - Use mcp_notion_notion_search with spec name or topic
...
```
1. Fetch spec page:
   - Use mcp_notion_notion_fetch with spec URL/ID from search results
...
1. Search for task database:
   - Use mcp_notion_notion_search to find "Tasks" or "Task Management" database
...
2. Fetch database schema:
   - Use mcp_notion_notion_fetch with database URL/ID
...
1. Create task in task database using mcp_notion_notion_create_pages
...
**"Can't find spec"**: Use mcp_notion_notion_search with spec name/topic, try broader terms, or ask user for URL
