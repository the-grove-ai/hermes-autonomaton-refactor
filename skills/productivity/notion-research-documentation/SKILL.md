...
1. **Search for relevant content**: Use `mcp_notion_notion_search` to find pages
2. **Fetch detailed information**: Use `mcp_notion_notion_fetch` to read full page content
3. **Synthesize findings**: Analyze and combine information from multiple sources
4. **Create structured output**: Use `mcp_notion_notion_create_pages` to write documentation
...
```
Use mcp_notion_notion_search with the research topic
...
```
Use mcp_notion_notion_fetch for each relevant page URL
