...
1. **Gather Notion context**: Use `mcp_notion_notion_search` to find related pages
2. **Fetch details**: Use `mcp_notion_notion_fetch` to read relevant content
3. **Enrich with research**: Use Claude's knowledge to add context, industry insights, or best practices
4. **Create internal pre-read**: Use `mcp_notion_notion_create_pages` for background context document (for attendees)
5. **Create external agenda**: Use `mcp_notion_notion_create_pages` for meeting agenda (shared with all participants)
...
```
Use mcp_notion_notion_search to find:
...
1. Fetch with mcp_notion_notion_fetch
...
```
Use mcp_notion_notion_create_pages for internal doc:
...
```
Use mcp_notion_notion_create_pages for meeting doc:
