...
class TestHermesTimeNow:
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["GROVE_TIMEZONE"] = "UTC"
        result = hermes_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["GROVE_TIMEZONE"] = "America/New_York"
        result = hermes_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["GROVE_TIMEZONE"] = "Mars/Olympus_Mons"
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["GROVE_TIMEZONE"] = "Europe/London"
    ...
    def test_returns_none_for_invalid(self):
        os.environ["GROVE_TIMEZONE"] = "Not/A/Timezone"
        tz = hermes_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When GROVE_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) GROVE_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("GROVE_TIMEZONE=" + os.environ.get("GROVE_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "GROVE_TIMEZONE=NOT_SET" in result["output"], (
            "GROVE_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When GROVE_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("GROVE_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        ...
        os.environ["GROVE_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_hermes_time_cache()
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["GROVE_TIMEZONE"] = "US/Eastern"
        _reset_hermes_time_cache()
