...
def isolated_home(tmp_path, monkeypatch):
    """Isolate GROVE_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".grove"
    home.mkdir()
...
class TestIntegrationWithModelsModule:
    ...
    def test_picker_nous_row_uses_manifest(self, tmp_path, monkeypatch):
        ...
        # ``GROVE_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` GROVE_HOME directly instead.
        import importlib
        from hermes_cli import model_catalog
        importlib.reload(model_catalog)
        try:
            ...
            active_home = Path(os.environ["GROVE_HOME"])
