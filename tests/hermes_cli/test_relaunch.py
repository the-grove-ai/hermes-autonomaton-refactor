...
class TestExtractInheritedFlags:
    def test_extracts_store_true_flags(self):
        argv = ["--yolo", "--ignore-rules", "chat"]
        assert relaunch_mod._extract_inherited_flags(argv) == ["--yolo", "--ignore-rules"]
...
class TestBuildRelaunchArgv:
    ...
    def test_preserves_inherited_flags(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_hermes_bin", lambda: "/usr/bin/hermes")
        original = ["--yolo", "--ignore-rules", "--profile", "work", "sessions", "browse"]
