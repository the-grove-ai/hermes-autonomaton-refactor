...
def test_spotify_interactive_setup_persists_client_id(
    ...
    """The wizard writes GROVE_SPOTIFY_CLIENT_ID to .env and returns the value."""
    monkeypatch.setenv("GROVE_HOME", str(tmp_path))
    ...
    assert "GROVE_SPOTIFY_CLIENT_ID=wizard-client-123" in env_text
    # Default redirect URI should NOT be persisted.
    assert "GROVE_SPOTIFY_REDIRECT_URI" not in env_text
...
def test_spotify_interactive_setup_empty_aborts(
    ...
    env_path = tmp_path / ".env"
    if env_path.exists():
        assert "GROVE_SPOTIFY_CLIENT_ID" not in env_path.read_text()
