...
def test_list_profiles_on_disk(tmp_path, monkeypatch):
    ...
    named profiles under ~/.grove/profiles/ that contain a config.yaml."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.delenv("GROVE_HOME", raising=False)
    profiles = tmp_path / ".grove" / "profiles"
...
def test_list_profiles_on_disk_custom_root(tmp_path, monkeypatch):
    """list_profiles_on_disk respects a custom GROVE_HOME root."""
...
def test_build_worker_context_renders_author_with_safe_framing(kanban_home):
    ...
    + "comment from worker" prefix so a misleading GROVE_PROFILE name
...
def test_default_spawn_auto_loads_kanban_worker_skill(kanban_home, monkeypatch):
    ...
    env = captured["env"]
    assert env.get("GROVE_KANBAN_TASK") == tid
    assert env.get("GROVE_PROFILE") == "some-profile"
...
def test_gateway_dispatcher_watcher_respects_env_override(monkeypatch):
    """GROVE_KANBAN_DISPATCH_IN_GATEWAY=0 disables without touching config."""
    import asyncio
    from gateway.run import GatewayRunner
    monkeypatch.setenv("GROVE_KANBAN_DISPATCH_IN_GATEWAY", "0")
...
def test_gateway_dispatcher_watcher_env_truthy_uses_config(monkeypatch):
    ...
    monkeypatch.setenv("GROVE_KANBAN_DISPATCH_IN_GATEWAY", "yes")
    monkeypatch.setattr(
        _cfg_mod, "load_config",
        lambda: {"kanban": {"dispatch_in_gateway": False}},
    )
