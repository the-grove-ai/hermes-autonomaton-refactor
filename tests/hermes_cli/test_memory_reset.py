...
- Profile-scoped reset (uses GROVE_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake GROVE_HOME with memory files."""
    ...
    (memories / "MEMORY.md").write_text(
        "§\nHermes repo is at ~/.grove/hermes-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
class TestMemoryReset:
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's GROVE_HOME."""
