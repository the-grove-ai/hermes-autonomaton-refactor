...
class TestDoctorMemoryProviderSection:
    ...
    def test_configured_provider_not_installed_shows_not_builtin(self, monkeypatch, tmp_path):
        # A configured-but-unavailable provider must not report built-in memory.
        out = self._run_doctor_and_capture(monkeypatch, tmp_path, provider="not_a_real_provider")
        assert "Memory Provider" in out
