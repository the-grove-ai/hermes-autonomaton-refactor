...
def test_mcp_allow_none_is_legacy_byte_for_byte(synth_caps):
    legacy = resolve_tools_for_tier(ALL_TOOLS, "research", "moderate", TAXONOMY,
                                    None, current_tier=3)
    flip_off = resolve_tools_for_tier(ALL_TOOLS, "research", "moderate", TAXONOMY,
                                      None, mcp_allow=None, current_tier=3)
...
def test_mcp_allow_discloses_only_matched_servers(synth_caps):
    res = resolve_tools_for_tier(ALL_TOOLS, "research", "moderate", TAXONOMY,
                                 None, mcp_allow={"notion"}, current_tier=3)
...
def test_mcp_allow_empty_withholds_all_mcp(synth_caps):
    res = resolve_tools_for_tier(ALL_TOOLS, "research", "moderate", TAXONOMY,
                                 None, mcp_allow=set(), current_tier=3)
...
def test_unknown_intent_fallback_full_on_t3(synth_caps):
    res = resolve_tools_for_tier(ALL_TOOLS, "unknown", "simple", TAXONOMY, None,
                                 current_tier=3)
    assert res.fallback is True
    got = _names(res.tools)
...
def test_tool_resolution_is_frozen(synth_caps):
    res = resolve_tools_for_tier(ALL_TOOLS, "analysis", "moderate", TAXONOMY,
                                 None, current_tier=2)
