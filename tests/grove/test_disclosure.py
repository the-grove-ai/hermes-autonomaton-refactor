...
MANIFEST = [
    ...
    _unit("[REDACTED]", "goal", keywords=("funding",), dock_goal="[REDACTED]"),
]
...
def test_resolve_pull_goal_unit_not_pullable_as_schema():
    ...
    text, defs = resolve_pull(MANIFEST, REGISTRY_TOOLS, "[REDACTED]")
    assert "error" in json.loads(text)
    assert defs == []
...
class _FakeDock:
    goals = (_FakeGoal("[REDACTED]"),)
    context_char_budget = 5000


def test_resolve_goal_record_returns_record():
    text = resolve_goal_record("[REDACTED]", dock=_FakeDock())
    payload = json.loads(text)
...
def test_resolve_goal_record_no_dock_is_loud():
    text = resolve_goal_record("[REDACTED]", dock=None, allow_load=False)
    assert "error" in json.loads(text)
...
def test_build_pull_tool_defs_embeds_index_and_omits_eager():
    ...
    assert "[REDACTED]" in rgc["function"]["description"]   # goal indexed
