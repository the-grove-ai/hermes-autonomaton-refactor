...
class TestM2RedMenu:
    def test_red_resolutions_membership(self):
        # propose-approve-deadlock-v1 Phase 1a added ``store_pending_approval``.
        # red-action-store-pending-v1 Phase B adds two more: ``denied_by_policy``
        # (catastrophic effects, never store-pending/executed) and the
        # RESERVED-EMPTY ``operator_identity_required`` (ExecutionIdentity dimension,
        # zero current members). None is an operator MENU choice — the Cancel/De-scope
        # menu below stays two choices.
        assert RED_RESOLUTIONS == (
            "cancel", "descoped", "store_pending_approval",
            "denied_by_policy", "operator_identity_required",
        )
