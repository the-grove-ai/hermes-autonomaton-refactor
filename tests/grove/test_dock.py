...
def test_seed_template_parses():
    ...
    assert ids == {"grv-001-[REDACTED]", "influencer-outreach", "[REDACTED]"}
    assert dock.context_char_budget == 4000
    by_id = {g.id: g for g in dock.goals}
    assert by_id["grv-001-[REDACTED]"].vector == "apex_strategic"
    assert by_id["[REDACTED]"].vector == "personal"
...
def test_resolve_goal_single_keyword_match(tmp_path):
    dock = load_dock(_write_dock(tmp_path, [
        _minimal_goal(id="house", keywords=["epoxy", "[REDACTED]"]),
...
def test_doctorow_example_against_seed():
    ...
    assert g.id == "grv-001-[REDACTED]"    # apex_strategic > strategic
