...
def backup_env(monkeypatch, tmp_path):
    """Isolate GROVE_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".grove"
    home.mkdir()
    (home / "skills").mkdir()
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under GROVE_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level GROVE_DIR picks up the tmp HOME."""
    import hermes_constants
    importlib.reload(hermes_constants)
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.grove/cron/jobs.json
