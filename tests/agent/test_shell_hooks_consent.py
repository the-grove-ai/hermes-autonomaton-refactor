...
three accept-hooks channels (--accept-hooks, GROVE_ACCEPT_HOOKS env,
...
def _isolated_home(tmp_path, monkeypatch):
    monkeypatch.setenv("GROVE_HOME", str(tmp_path / "hermes_home"))
    monkeypatch.delenv("GROVE_ACCEPT_HOOKS", raising=False)
...
class TestNonTTYFlow:
    ...
    def test_no_tty_with_env_accepts(self, tmp_path, monkeypatch):
        ...
        script = _write_hook_script(tmp_path)
        plugins._plugin_manager = plugins.PluginManager()
        monkeypatch.setenv("GROVE_ACCEPT_HOOKS", "1")
