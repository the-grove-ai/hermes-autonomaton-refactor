...
def test_curator_slot_is_canonical_aux_task():
    ...
    Two sources of truth, all checked by the shared registry test
    (test_aux_config.py) for the main tasks — this test pins `curator`
    specifically so the unification doesn't silently regress.  (The former
    web_server _AUX_TASK_SLOTS and web/ ModelsPage.tsx sources were removed
    with the dashboard stack in hermes-severance-v1.)
    """
