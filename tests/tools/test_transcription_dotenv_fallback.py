...
``~/.grove/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.grove/.env`` would be told
...
class TestEndToEndRegressionGuard:
    ...
    ``~/.grove/.env`` carrying the key while ``os.environ`` does not.
