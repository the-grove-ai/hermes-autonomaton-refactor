...
def isolated_session(monkeypatch):
    ...
    monkeypatch.setenv("GROVE_SESSION_KEY", session_key)
...
class TestCliPathFiresHooks:
    """CLI-interactive approval path: GROVE_INTERACTIVE is set, the
...
class TestGatewayPathFiresHooks:
    """Async gateway approval path: GROVE_GATEWAY_SESSION is set and a
