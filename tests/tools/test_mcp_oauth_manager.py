...
def test_hermes_provider_subclass_exists():
    ...
    assert issubclass(_GROVE_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_hermes_provider_subclass(tmp_path, monkeypatch):
    ...
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _GROVE_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("GROVE_HOME", str(tmp_path))
    ...
    assert _GROVE_PROVIDER_CLS is not None
    assert isinstance(provider, _GROVE_PROVIDER_CLS)
