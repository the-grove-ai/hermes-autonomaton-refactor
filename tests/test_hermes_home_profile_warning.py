...
When GROVE_HOME is unset but an active_profile file indicates a non-default
profile is active, get_hermes_home() should:
  1. STILL return ~/.grove (raising would brick 30+ module-level callers)
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        """Classic mode: no active_profile file → silent, returns ~/.grove."""
        result = fresh_constants.get_hermes_home()
        assert result == tmp_path / ".grove"
        assert "GROVE_HOME fallback" not in capsys.readouterr().err

    def test_default_active_profile_no_warning(
        self, fresh_constants, tmp_path, capsys
    ):
        """active_profile=default → still no warning, returns ~/.grove."""
        hermes_dir = tmp_path / ".grove"
        hermes_dir.mkdir()
    ...
    def test_named_profile_unset_home_warns_once(
        ...
        """active_profile=coder + GROVE_HOME unset → warn loudly, still return fallback."""
        hermes_dir = tmp_path / ".grove"
        hermes_dir.mkdir()
        ...
        assert err.count("GROVE_HOME fallback") == 1
        assert "'coder'" in err
        assert "#18594" in err
        ...
        assert "GROVE_HOME fallback" not in err2

    def test_hermes_home_set_suppresses_warning(
        self, fresh_constants, tmp_path, capsys, monkeypatch
    ):
        """Even if active_profile is 'coder', setting GROVE_HOME suppresses warning."""
        profile_dir = tmp_path / ".grove" / "profiles" / "coder"
        profile_dir.mkdir(parents=True)
        (tmp_path / ".grove" / "active_profile").write_text("coder\n")
