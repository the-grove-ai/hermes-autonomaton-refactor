...
class TestGetDefaultHermesRoot:
    ...
    def test_no_hermes_home_returns_native(self, tmp_path, monkeypatch):
        """When GROVE_HOME is not set, returns ~/.grove."""
        monkeypatch.delenv("GROVE_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_hermes_root() == tmp_path / ".grove"

    def test_hermes_home_is_native(self, tmp_path, monkeypatch):
        """When GROVE_HOME = ~/.grove, returns ~/.grove."""
        native = tmp_path / ".grove"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("GROVE_HOME", str(native))
        assert get_default_hermes_root() == native

    def test_hermes_home_is_profile(self, tmp_path, monkeypatch):
        """When GROVE_HOME is a profile under ~/.grove, returns ~/.grove."""
        native = tmp_path / ".grove"
    ...
    def test_hermes_home_is_docker(self, tmp_path, monkeypatch):
        """When GROVE_HOME points outside ~/.grove (Docker), returns GROVE_HOME."""
    ...
    def test_hermes_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any GROVE_HOME outside ~/.grove is treated as the root."""
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (GROVE_HOME=<root>/profiles/<name>),
